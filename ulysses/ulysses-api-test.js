// Ulysses Test Script
require("ulysses-v2.js");
var yesNo = ["Yes", "No"];
var ok = ["OK"];
var testID = "0sKFiy02n2y-QwQbXCIYDA";
var testGroupID = "_irtj5J8siY8DXj0E4ckmA";
var testTrashID = "3x7EzNTdo2EHis-02kHh8A";
var loremLong =
  "\n\nLorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.";
var loremShort =
  "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.";
var base64IMG =
  "";

var ulysses = new Ulysses();
ulysses.debug = true;

// API Commands to implement
function prompt(message, buttons) {
  // Prompt
  /* --------------------- */

  var p = Prompt.create();
  p.isCancellable = false;
  p.title = "Ulysses API Test";
  p.message = message;
  for (var button of buttons) {
    p.addButton(button);
  }
  var con = p.show();

  return p.buttonPressed;
}

function ulyssesAPITest() {
  // Test: get-version
  var version = ulysses.getVersion(); // Done
  prompt(
    "API Version: " + version.apiVersion + "\nBuild: " + version.buildNumber,
    ok
  );
  if (prompt("Did you see the API version and build number?", yesNo) == "No") {
    return false;
  }

  // Test: new-group
  // creates test groups
  ulysses.newGroup("Ulysses API Test");
  var testParentGroup = ulysses.targetID;
  ulysses.newGroup("Group 1", "Ulysses API Test");
  var testGroup1 = ulysses.targetID;
  ulysses.newGroup("Sub Group 1", "/Ulysses API Test/Group 1/");
  ulysses.newGroup("Group 2", testParentGroup, 1);
  var testGroup2 = ulysses.targetID;
  if (
    prompt("In Ulysses, do you see 3 new groups and 1 new sub group?", yesNo) ==
    "No"
  ) {
    return false;
  }

  // Test: new-sheet
  var sheet1 = "# Sheet 1" + loremLong;
  ulysses.newSheet(sheet1, testParentGroup);
  var testSheet = ulysses.targetID;
  ulysses.open(testParentGroup);

  if (
    prompt("In Ulysses, do you see a new sheet titled 'Sheet 1?'", yesNo) ==
    "No"
  ) {
    return false;
  }

  // Test: insert
  ulysses.insert(testSheet, loremShort, "marmdown", "end", "enclose");
  if (prompt("In Ulysses, has text been added to Sheet 1?", yesNo) == "No") {
    return false;
  }

  //Test: attach-note
  ulysses.attachNote(loremShort, testSheet);
  ulysses.attachNote(loremLong, testSheet);
  if (
    prompt("In Ulysses, does Sheet 1 have two notes attached?", yesNo) == "No"
  ) {
    return false;
  }

  // Test: update-note
  ulysses.updateNote(testSheet, loremShort, 1);
  if (
    prompt("In Ulysses, has the second note been shortened?", yesNo) == "No"
  ) {
    return false;
  }

  // Test: remove-note
  ulysses.removeNote(testSheet, 1);
  if (prompt("In Ulysses, has the second note been removed?", yesNo) == "No") {
    return false;
  }

  // Test: attach-image
  ulysses.attachImage(testSheet, base64IMG, "png");
  if (
    prompt("In Ulysses,has an image been attached to sheet 1?", yesNo) == "No"
  ) {
    return false;
  }

  // Test: attach-keywords
  var keywords = ["Test", "Testing"];
  ulysses.attachKeywords(keywords, testSheet);
  if (prompt("In Ulysses, does Sheet 1 have two keywords?", yesNo) == "No") {
    return false;
  }

  // Test: remove-keywords
  ulysses.removeKeywords(testSheet, keywords);
  if (prompt("In Ulysses, have the keywords been removed?", yesNo) == "No") {
    return false;
  }

  // Test: open-recent
  //  ulysses.openRecent();

  // Test: open-favorites
  //  ulysses.openFavorites();

  // Test: open-all
  ulysses.openAll();

  // Test: trash
  ulysses.trash(testParentGroup);
  //ulysses.trash(testGroup1);
  //ulysses.trash(testGroup2);
  if (
    prompt("Have all the test groups and sheets been removed?", yesNo) == "No"
  ) {
    return false;
  }
  return true;
}

if (ulyssesAPITest()) {
  prompt("API is working as expected!", ok);
} else {
  context.fail();
  prompt("Something went wrong, check the console logs.", ok);
}
