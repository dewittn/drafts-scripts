// Ulysses Test Script
require("ulysses-v2.js");
const yesNo = ["Yes", "No"];
const ok = ["OK"];
const loremLong =
  "\n\nLorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.";
const loremShort =
  "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.";
const base64IMG =
  "";

var ulysses = new Ulysses();
ulysses.debug = true;

// API Commands to implement
function prompt(message, buttons) {
  // Prompt
  /* --------------------- */

  let p = Prompt.create();
  p.isCancellable = false;
  p.title = "Ulysses API Test";
  p.message = message;
  for (var button of buttons) {
    p.addButton(button);
  }
  p.show();

  return p.buttonPressed;
}

function ulyssesAPITest() {
  // Test: get-version
  const version = ulysses.getVersion(); // Done
  prompt("API Version: " + version.apiVersion + "\nBuild: " + version.buildNumber, ok);
  if (prompt("Did you see the API version and build number?", yesNo) == "No") {
    return false;
  }

  // Test: new-group
  // creates test groups

  const testParentGroup = ulysses.newGroup("Ulysses API Test");
  const testGroup1 = ulysses.newGroup("Group 1", "Ulysses API Test");
  ulysses.newGroup("Sub Group 1", "/Ulysses API Test/Group 1/");
  const testGroup2 = ulysses.newGroup("Group 2", testParentGroup.targetId, 1);
  if (prompt("In Ulysses, do you see 3 new groups and 1 new sub group?", yesNo) == "No") {
    return false;
  }

  // Test: new-sheet
  const sheet1 = "# Sheet 1" + loremLong;
  const testSheet = ulysses.newSheet(sheet1, testParentGroup.targetId);
  ulysses.open(testParentGroup.targetId);

  if (prompt("In Ulysses, do you see a new sheet titled 'Sheet 1?'", yesNo) == "No") {
    return false;
  }

  // Test: insert
  ulysses.insert(testSheet.targetId, loremShort, "marmdown", "end", "enclose");
  if (prompt("In Ulysses, has text been added to Sheet 1?", yesNo) == "No") {
    return false;
  }

  //Test: attach-note
  ulysses.attachNote(testSheet.targetId, loremShort);
  ulysses.attachNote(testSheet.targetId, loremLong);
  if (prompt("In Ulysses, does Sheet 1 have two notes attached?", yesNo) == "No") {
    return false;
  }

  // Test: update-note
  ulysses.updateNote(testSheet.targetId, loremShort, 1);
  if (prompt("In Ulysses, has the second note been shortened?", yesNo) == "No") {
    return false;
  }

  // Test: remove-note
  ulysses.removeNote(testSheet.targetId, 1);
  if (prompt("In Ulysses, has the second note been removed?", yesNo) == "No") {
    return false;
  }

  // Test: attach-image
  ulysses.attachImage(testSheet.targetId, base64IMG, "png");
  if (prompt("In Ulysses,has an image been attached to sheet 1?", yesNo) == "No") {
    return false;
  }

  // Test: attach-keywords
  const keywords = ["Test", "Testing"];
  ulysses.attachKeywords(testSheet.targetId, keywords);
  if (prompt("In Ulysses, does Sheet 1 have two keywords?", yesNo) == "No") {
    return false;
  }

  // Test: remove-keywords
  ulysses.removeKeywords(testSheet.targetId, keywords);
  if (prompt("In Ulysses, have the keywords been removed?", yesNo) == "No") {
    return false;
  }

  // Test: set-group-title
  ulysses.setGroupTitle("Group 1", "Group 1: Renamed");
  ulysses.setGroupTitle("/Ulysses API Test/Group 1: Renamed/Sub Group 1/", "Sub Group 1: Renamed");
  ulysses.setGroupTitle(testGroup2.targetId, "Group 2: Renamed");
  if (prompt("In Ulysses, have all three groups been renamed?", yesNo) == "No") {
    return false;
  }

  // Test: set-sheet-title
  ulysses.setSheetTitle(testSheet.targetId, "Sheet 1: Renamed", "heading2");
  if (prompt("In Ulysses, has Sheet 1 been renamed?", yesNo) == "No") {
    return false;
  }

  // Test: move
  ulysses.move(testSheet.targetId, testGroup2.targetId);
  if (prompt("In Ulysses, has Sheet 1 been moved to Group 2?", yesNo) == "No") {
    return false;
  }

  // Test: copy
  ulysses.copy(testSheet.targetId, testGroup1.targetId);
  if (prompt("In Ulysses, Sheet 1 been copied back to Group 1", yesNo) == "No") {
    return false;
  }

  // Test: get-item
  var item = ulysses.getItem(testGroup1.targetId, "No");
  prompt(JSON.stringify(item), ok);
  if (prompt("Was the informatino about Group 1 displayed correctly?", yesNo) == "No") {
    return false;
  }

  // Test: get-root-items
  var rootItems = ulysses.getRootItems("No");
  prompt(JSON.stringify(rootItems), ok);
  if (prompt("Was the informatino about the root items displayed correctly?", yesNo) == "No") {
    return false;
  }

  // Test: read-sheet
  var sheet = ulysses.readSheet(testSheet.targetId, "Yes");
  prompt(JSON.stringify(sheet), ok);
  if (prompt("Was the informatino about Sheet 1 displayed correctly?", yesNo) == "No") {
    return false;
  }

  // Test: open-recent
  ulysses.openRecent();
  if (prompt("Did Ulysses open to Recent Items?", yesNo) == "No") {
    return false;
  }

  // Test: open-favorites
  ulysses.openFavorites();
  if (prompt("Did Ulysses open to Favorites?", yesNo) == "No") {
    return false;
  }

  // Test: open-all
  ulysses.openAll();
  if (prompt("Did Ulysses open to All Items?", yesNo) == "No") {
    return false;
  }

  // Test: trash
  ulysses.trash(testParentGroup.targetId);
  //ulysses.trash(testGroup1);
  //ulysses.trash(testGroup2);
  if (prompt("Have all the test groups and sheets been removed?", yesNo) == "No") {
    return false;
  }
  return true;
}

if (ulyssesAPITest()) {
  prompt("API is working as expected!", ok);
} else {
  context.fail();
  prompt("Something went wrong, check the console logs.", ok);
}
